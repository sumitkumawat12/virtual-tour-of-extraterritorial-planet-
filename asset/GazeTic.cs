...
public class GazeTic : MonoBehaviour
{
    private float gazeTimer = 0f;
    public float gazeTime = 2f; // Time in seconds to trigger the scene change
    public GameObject sphere4;  // Reference to Sphere 3
    public GameObject sphere5;  // Reference to Sphere 4
    public Transform sphere5Center; // Reference to the center position of Sphere 4
    ...
    void Start()
    {
        sphere4.SetActive(true);  // Start with Sphere 3 active
        sphere5.SetActive(false); // Sphere 4 is inactive initially
    }
    ...
    void MoveCameraToCenter()
    {
        ...
        Player.transform.position = sphere5Center.position;
        Player.transform.rotation = sphere5Center.rotation;
